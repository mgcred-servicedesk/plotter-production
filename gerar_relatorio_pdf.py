...
from src.reports.pdf_generator import gerar_relatorio_pdf
from src.reports.pdf_resumo import gerar_relatorio_resumo_pdf
from src.reports.pdf_detalhado import gerar_relatorio_detalhado_pdf
...
def main(mes=3, ano=2026):
    """Gera relatórios PDF completos (Resumo e Detalhado)."""
    print(f"\n{'#'*80}")
    ...
    print(f"\n1. Gerando Relatório RESUMO...")
    arquivo_resumo = gerar_relatorio_resumo_pdf(
        df, mes, ano, df_metas, dia_atual=dia_atual
    )
    print(f"   ✓ Relatório Resumo gerado: {arquivo_resumo}")
    
    print(f"\n2. Gerando Relatório DETALHADO...")
    arquivo_detalhado = gerar_relatorio_detalhado_pdf(
        df, mes, ano, df_metas, dia_atual=dia_atual
    )
    print(f"   ✓ Relatório Detalhado gerado: {arquivo_detalhado}")
    
    print(f"\n3. Gerando Relatório ORIGINAL (compatibilidade)...")
    arquivo_original = gerar_relatorio_pdf(df, mes, ano, df_metas=df_metas)
    print(f"   ✓ Relatório Original gerado: {arquivo_original}")
    ...
    return arquivo_resumo, arquivo_detalhado, arquivo_original
