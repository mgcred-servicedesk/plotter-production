        ...
        for _, row_data in tabela.iterrows():
            ...
            for col_idx, (col_name, value) in enumerate(zip(headers, row_data), 1):
                ...
                if 'Meta' in col_name and col_name != 'Loja':
                    ...
                    any(x in col_name for x in [
                        'CNC', 'SAQUE', 'CLT', 'CONSIGNADO', 'PACK',
                        'Projeção', 'Média DU', 'Ticket Médio',
                    ])
                    and 'Loja' not in col_name and '%' not in col_name
                ):
        ...
        for _, row_data in tabela.iterrows():
            ...
            for col_idx, (col_name, value) in enumerate(zip(headers, row_data), 1):
                ...
                if 'Meta' in col_name and col_name != 'Loja':
                    ...
                    any(x in col_name for x in [
                        'Valor', 'Média DU', 'Ticket Médio', 'Projeção',
                    ])
                    and 'Loja' not in col_name
                ):
                    cell.number_format = 'R$ #,##0.00'
                elif '%' in col_name:
                    cell.number_format = '0.00"%"'
...
def criar_aba_resumo_geral(wb, df, df_metas, mes, ano, dia_atual=None, df_supervisores=None):
    ...
    cell_top10 = ws.cell(row=row_atual, column=1, value="TOP 10 CONSULTORES - POR ATINGIMENTO META PRATA")
    cell_top10.font = Font(bold=True, size=12)
    row_atual += 1
    ...
    cell_rank_regiao = ws.cell(row=row_atual, column=1, value="RANKING DE REGIÃO - POR ATINGIMENTO META PRATA")
    cell_rank_regiao.font = Font(bold=True, size=12)
    row_atual += 1
