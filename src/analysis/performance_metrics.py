...
def analisar_consultor(
    ...
    if 'PRODUTO' in df_consultor.columns:
        ...
        modo_produto = df_consultor['PRODUTO'].mode()
        analise['produto_mais_vendido'] = modo_produto[0] if len(modo_produto) > 0 else ''
...
def comparar_consultor_com_media(
    ...
    comparacao = {
        ...
        'percentual_vs_media': (
            ((analise_consultor['total_pontos'] - media_pontos) / media_pontos * 100)
            if media_pontos > 0 else 0
        ),
...
def calcular_consistencia_consultor(
    ...
    consistencia = {
        ...
        'coeficiente_variacao': (
            (evolucao['pontos'].std() / evolucao['pontos'].mean() * 100)
            if evolucao['pontos'].mean() > 0 else 0
        ),
