...
def criar_relatorio_mix(df, df_metas, ano, mes, dia_atual=None):
    ...
    totais = {
        ...
        'Atingimento (%)': (
            (df_mix['Qtd Realizada'].sum() / df_mix['Meta'].sum() * 100)
            if df_mix['Meta'].sum() > 0 else 0
        ),
        'Ticket Médio': (
            df_mix['Valor Realizado'].sum() / df_mix['Qtd Realizada'].sum()
            if df_mix['Qtd Realizada'].sum() > 0 else 0
        ),
