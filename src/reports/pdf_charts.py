...
def criar_grafico_barras_top10(df, coluna_nome, coluna_valor, titulo, cor=None):
    ...
    ax.barh(df_top[coluna_nome], df_top[coluna_valor], color=cor, alpha=0.8)
...
def criar_grafico_barras_regioes(df_regioes, titulo="Performance por Região"):
    ...
    if pontos:
        ...
        ax.bar(x - width / 2, pontos_norm, width, label='Pontos (normalizado)',
               color=COLORS['primary'], alpha=0.8)

    if atingimento:
        ax.bar(x + width / 2, atingimento, width, label='Atingimento %',
