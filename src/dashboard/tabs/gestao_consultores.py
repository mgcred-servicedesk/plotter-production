...
def _render_resumo(
    ...
    # nada (media de medias), entao o card exibe a media do grupo.
    if COL_TOTAL in resultado.columns and not resultado.empty:
        if metrica in (METRICA_TICKET, METRICA_SHARE):
...
def render_tab_gestao(
    ...
    """Renderiza a aba de gestao por faixa de valor/quantidade.

    ``carregar_intervalo(ini, fim, campo) -> (df, aviso)`` e injetado
    por ``app.py`` e ja devolve os contratos com RLS e filtros da
    sidebar aplicados. Sem ele, o periodo personalizado fica indisponivel
    e a aba segue no mes da sidebar.
    """
    ...
    if ignorados:
        st.warning(
            ...
            + ", ".join(ignorados)
            + ". "
            + _motivo_meta_indisponivel(periodo_ativo, nivel, ignorados)
        )
    ...
    colunas_qtd = [
        c for c in (*ROTULOS_ACELERADORES, _COL_LACUNA_ACEL)
