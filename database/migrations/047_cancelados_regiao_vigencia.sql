-- =====================================================
-- Migracao 047: obter_cancelados_classificados com regiao
--               point-in-time e regiao_atual
--
-- Estagio 2b. Ajusta a RPC (ancora 039) para o versionamento de
-- regiao:
--   * canc: `regiao` (point-in-time) e a nova `regiao_atual` vem da
--     view v_contratos_cancelados (migration 044);
--   * paga: a regiao usada no MATCHING de recuperacao passa a ser a
--     vigente na venda (data_cadastro, via loja_regiao_vigencia), em
--     vez da regiao atual da loja. Como cancelado e recuperacao caem
--     numa janela de <=7 dias e a regiao so muda em fronteira de
--     periodo, a semantica de "recuperada em OUTRA regiao" fica
--     equivalente (so difere no straddle exato da fronteira, onde o
--     comportamento novo e o correto);
--   * saida ganha `regiao_atual` (RLS client-side do gerente).
--
-- Fora isso, IDENTICO a 039: mesma classificacao (redigitada /
-- recuperada / liquido), mesmas flags de oportunidade perdida,
-- matching por cliente_norm+categoria em 7 dias, set-based, e
-- SET statement_timeout = '15000'. Migration 039 e imutavel; esta a
-- substitui via DROP + CREATE (o RETURNS TABLE ganhou uma coluna).
--
-- No-op ate o 1o remanejamento: COALESCE(vigencia, l.regiao_id) e a
-- linha de vigencia aberta desde 2020 reproduzem a regiao atual.
--
-- ⚠️ PERFORMANCE: a RPC ja opera perto do statement_timeout de 15s.
-- A resolucao de regiao por vigencia adiciona um LATERAL (LIMIT 1)
-- indexado sobre uma tabela minima (1-2 linhas/loja). VALIDAR com
-- EXPLAIN ANALYZE (secao final) antes de aplicar o Estagio 3.
--
...
BEGIN
    ...
    RETURN QUERY
    WITH canc AS (
        -- Cancelados da janela; regiao/regiao_atual da view (044).
        -- nome normalizado (cliente_norm) so p/ o matching.
        SELECT
    ...
    paga AS (
        ...
        -- recuperacao. regiao = vigente na venda (data_cadastro) via
        -- loja_regiao_vigencia, casando com a regiao point-in-time do
        -- cancelado. Filtro de data limita o scan.
        SELECT
            c.cliente_norm AS nome_norm,
...
COMMENT ON FUNCTION obter_cancelados_classificados(INTEGER, INTEGER) IS
    ...
    'regiao = vigente na venda (point-in-time via loja_regiao_vigencia), '
    'usada tambem no matching; regiao_atual = regiao corrente da loja '
    '(RLS client-side). Migration 047 (base 039 + versionamento regiao).';

-- Acesso de execucao para os papeis do app (preserva GRANT da 039).
GRANT EXECUTE ON FUNCTION obter_cancelados_classificados(INTEGER, INTEGER)
    TO anon, authenticated;


-- ===========================================
-- Validacao de performance (EXPLAIN ANALYZE) — rodar no Supabase
-- ANTES de aplicar o Estagio 3 (remanejamento real).
--
--   EXPLAIN (ANALYZE, BUFFERS, VERBOSE)
--   SELECT * FROM obter_cancelados_classificados(6, 2026);
--
-- Comparar "Execution Time" com a 039 (deve manter folga sobre os
-- 15s). Sanidade de equivalencia PRE-remanejamento (contagens
-- devem bater com a 039):
--
