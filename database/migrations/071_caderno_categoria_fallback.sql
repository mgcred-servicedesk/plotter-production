-- Migration 071: corrige o fallback de categoria no contrato do Caderno.
--
-- A 070 consumia apenas categoria_codigo da view. O dashboard, entretanto,
-- aplica fallback por tipo_produto quando produtos.categoria_id esta NULL.
-- Sem o mesmo fallback, CLT e Antecipacao de Beneficio desapareciam do MIX
-- e seus contratos ficavam sem multiplicador de pontos.
--
-- A 070 ja foi aplicada e permanece imutavel. Esta migration substitui
-- somente a funcao, preservando assinatura, grants e formato do JSON.
        ...
        CASE
            ...
            WHEN v.categoria_canonica IN ('CONSIG_BMG', 'CONSIG_ITAU', 'CONSIG_C6')
             ...
             AND nullif(regexp_replace(coalesce(v.prazo, ''), '[^0-9]', '', 'g'), '')::integer < 96
                THEN 'FLEX'
            WHEN v.modalidade_tabela = 'NORMAL'
                THEN 'NORMAL'
            ELSE 'DESCONHECIDA'
...
produto_ranking AS (
    SELECT
        ...
        row_number() OVER (
            ...
            ORDER BY "paidEffective" DESC, store
        ) AS position,
        store,
        manager,
        "paidEffective",
...
COMMENT ON FUNCTION public.obter_caderno_fechamento(integer, integer) IS
    'Contrato agregado v1.1 do Caderno de Fechamento. Usa regiao historica '
    'da competencia, valor_consolidado, classificacao NORMAL/FLEX com '
    'diagnostico explicito, fallback de categoria equivalente ao dashboard, '
    'Portabilidade incorporada ao Consignado e ranking por produto/loja.';
