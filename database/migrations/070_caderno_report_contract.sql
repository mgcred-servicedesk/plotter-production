-- Migration 070: contrato agregado v1 do Caderno de Fechamento.
-- Registrada no repositorio proprietario do schema; a presenca deste
-- arquivo nao implica que a migration ja tenha sido aplicada no Supabase.
-- Requer migrations 043, 045, 051, 061 e 067 do plotter-production.
...
pontos_categoria AS (
    SELECT * FROM public.obter_pontuacao_periodo(p_mes, p_ano)
),
contratos_classificados AS (
    SELECT
        ...
        CASE
            WHEN v.categoria_codigo = 'PORTABILIDADE' THEN
                ...
                END
            ELSE v.categoria_codigo
        END AS categoria_pontos,
        CASE
            WHEN upper(btrim(coalesce(prod.tipo_operacao, ''))) = 'FLEX'
                THEN 'FLEX'
            WHEN v.categoria_codigo IN ('CONSIG_BMG', 'CONSIG_ITAU', 'CONSIG_C6')
             AND upper(btrim(coalesce(v.subtipo, ''))) = 'NOVO'
             AND nullif(regexp_replace(coalesce(v.prazo, ''), '[^0-9]', '', 'g'), '')::integer < 96
                THEN 'FLEX'
            WHEN upper(btrim(coalesce(prod.tipo_operacao, ''))) = 'NORMAL'
                THEN 'NORMAL'
            ELSE 'DESCONHECIDA'
        END AS classificacao_validade,
        CASE
            WHEN v.categoria_codigo IN ('CNC', 'CNC_13', 'SUPER_CONTA') THEN 'CNC'
            WHEN v.categoria_codigo = 'FGTS' THEN 'FGTS'
            WHEN v.categoria_codigo IN ('SAQUE', 'SAQUE_BENEFICIO') THEN 'SAQUE'
            WHEN v.categoria_codigo = 'CONSIG_PRIV' THEN 'CLT'
            WHEN v.categoria_codigo = 'ANT_BENEF' THEN 'ANTECIP_BENEFICIO'
            WHEN v.categoria_codigo IN ('CONSIG_BMG', 'CONSIG_ITAU', 'CONSIG_C6', 'PORTABILIDADE') THEN 'CONSIGNADO'
        END AS grupo_mix,
        coalesce(v.conta_pontuacao, true) AS conta_pontuacao
    FROM public.v_contratos_dashboard v
    JOIN periodo per ON per.id = v.periodo_id
...
base AS (
    ...
    LEFT JOIN pontos_categoria pc ON pc.categoria_codigo = c.categoria_pontos
),
metas AS (
    SELECT * FROM public.obter_metas_geral_loja(p_mes, p_ano)
),
metas_loja AS (
    SELECT
        loja,
...
headcount AS (
    SELECT count(DISTINCT c.nome)::integer AS total
    FROM public.consultores c
    JOIN public.lojas l ON l.id = c.loja_id
    WHERE upper(btrim(coalesce(c.status, ''))) LIKE 'ATIVO%'
      AND coalesce(l.ativo, true)
      AND NOT EXISTS (
          SELECT 1 FROM public.supervisores s
          WHERE upper(btrim(s.nome)) = upper(btrim(c.nome))
      )
),
resumo AS (
    SELECT
...
COMMENT ON FUNCTION public.obter_caderno_fechamento(integer, integer) IS
    'Contrato agregado v1 do Caderno de Fechamento. Usa regiao historica '
    'da competencia, valor_consolidado, classificacao NORMAL/FLEX com '
    'diagnostico explicito e Portabilidade incorporada ao Consignado.';
