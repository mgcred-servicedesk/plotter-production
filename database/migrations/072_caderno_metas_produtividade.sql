-- Migration 072: metas, pontuacao mensal e produtividade por loja no Caderno.
--
-- Amplia o contrato criado nas migrations 070/071 sem alterar as migrations
-- ja aplicadas. A serie de pontos usa a regiao vigente em cada competencia;
-- por isso um remanejamento nao reatribui retroativamente a producao.
...
headcount_loja AS (
    SELECT
        l.nome AS loja,
        count(DISTINCT c.nome)::integer AS total
...
COMMENT ON FUNCTION public.obter_caderno_fechamento(integer, integer) IS
    'Contrato agregado v1.2 do Caderno de Fechamento. Inclui serie mensal '
    'de pontos/metas com regiao historica por competencia, niveis Prata/Ouro '
    'e produtividade PAGO por consultor ativo em cada loja. VAI E VEM fora '
    'do escopo. Mantem regras NORMAL/FLEX e MIX das migrations 070/071.';
