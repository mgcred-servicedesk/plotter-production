-- Migration 074: VAI E VEM fora do eixo comparativo do Caderno.
--
-- As migrations 070-073 ja foram aplicadas e permanecem imutaveis. Esta
-- versao substitui somente a funcao, preservando assinatura e grants.
--
-- VAI E VEM nao e loja de venda: e o ponto de backoffice que registra, por
-- bancos parceiros, contratos com impedimento na loja. Quando a proposta e
-- paga, a producao e revertida para a loja que iniciou a negociacao; fica
-- sob o Vai e Vem apenas o residuo de contratos em condicoes especificas
-- para pessoas especificas.
--
-- Decisao de negocio para o Caderno de Fechamento, documento de premiacao:
--   * fora de todo eixo COMPARATIVO — ranking, contagem de lojas, serie
--     mensal de pontos, rankings por produto, produtividade e headcount;
--   * dentro de todo eixo ABSOLUTO — producao paga efetiva, producao valida
--     e MIX seguem contando o valor.
-- O resumo passa a expor 'backoffice' para o relatorio declarar a diferenca
-- entre o total pago e a soma da coluna do ranking, sem segunda consulta.
--
-- Isto DIVERGE do dashboard DE PROPOSITO: la a loja Vai e Vem permanece no
-- ranking de lojas (docs/agents/business-rules.md, "Lojas de backoffice"),
-- porque aquele ranking e analitico. Aqui o ranking distribui premiacao.
-- Nao "alinhar" os dois sem revisitar a decisao de negocio.
...
headcount_loja AS (
    ...
    WHERE (
      ...
      AND upper(btrim(coalesce(l.nome, '')))
          NOT IN (SELECT nome FROM lojas_backoffice)
      AND NOT EXISTS (
          SELECT 1
